...
## Introduction
This repo contains an example of a Tekton pipeline that builds and deploys an application created with [Appsody](https://github.com/appsody/appsody) to a Kubernetes cluster. The application is deployed via the [Appsody Operator](https://github.com/appsody/appsody-operator), which the pipeline also installs for you. 
...
1) Your code repository includes a manifest file called `app-deploy.yaml`. We'll discuss this aspect in more detail later on.
...
As we mentioned earlier, the pipeline is designed to deploy your application to the Kubernetes cluster using a provided deployment manifest. The pipeline expects a deployment manifest located within your project - specifically, it expects to run `kubectl apply` against a file named `app-deploy.yaml`.  This file can be generated by the developer, by using the Appsody CLI and running `appsody deploy --generate-only`, as described in [the Appsody documentation](https://appsody.dev/docs).
